...
class AdminDashboardTest extends TestCase
{
    ...
    public function test_admin_request_show_creates_missing_drive_folder_for_new_request(): void
    {
        $admin = User::factory()->create();
        $admin->forceFill(['is_admin' => true])->save();
        Sanctum::actingAs($admin);
        ...
        $this->mock(GoogleDriveClient::class, function ($mock): void {
            ...
            $mock->shouldReceive('ensureFolderPath')->once()->andReturn('folder-new');
        });

        $this->getJson("/api/admin/requests/{$serviceRequest->id}")
            ->assertOk()
            ->assertJsonPath('data.google_drive_folder_id', 'folder-new');
    }
