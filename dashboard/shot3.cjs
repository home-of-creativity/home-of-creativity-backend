...
(async () => {
  ...
  await mpage.goto("http://127.0.0.1:5173/dashboard/login", { waitUntil: "networkidle" });
  ...
  await Promise.all([
    mpage.waitForURL("http://127.0.0.1:5173/dashboard/", { timeout: 8000 }).catch(() => {}),
  ...
  await page.goto("http://127.0.0.1:5173/dashboard/requests/37", { waitUntil: "networkidle" });
