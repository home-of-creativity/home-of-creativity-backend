...
Request detail shows the current Sham Cash QR thumb and links to `/payments` to replace it. Client receipts and re-request appear only in `awaiting_payment`. The Drive folder is a `drive.google.com` link (request folder, or the Hoc Client parent while missing). **إنشاء مجلد Drive** is always shown until the request folder exists; confirming payment also creates it immediately. The Google service account JSON is **not** uploaded from the dashboard. Deploy writes `storage/app/private/google-sa.json` from GitHub secret `GOOGLE_SERVICE_ACCOUNT_JSON_B64` (rsync excludes `storage/app/`). After a quotation approve/reject, sales get a `tg://user?id=` private chat link (dashboard + staff notify). Writes from the dashboard push to Odoo immediately. **Clients** GET stays SQL-fast; complete Telegram rows missing `odoo_lead_id` create or reuse an opportunity on the existing sales team **تلغرام** and stage **تلغرام** (no second team). Deploy restarts the scheduler and runs `odoo:reconcile` so missing cards land without re-adding the client. Telegram stubs without a complete profile stay hidden and are not pushed. A client keeps one `odoo_partner_id`; quotations reuse it instead of creating a new partner. Person names are not overwritten by the partner/company label. Quotation/invoice tabs read Odoo live and refresh every 15s; request detail hydrates quotation/invoice ids, amounts, and live state from Odoo and refreshes every 15s. POST/PUT/DELETE clients push partner + lead the same way. **Employees** on php-fpm GET pull `hr.employee` (60s cache) and prune locals missing from a complete list; `artisan serve` reads SQL only so nav/overview stay fast. Confirm payment stays here: staff review the receipt, enter the amount actually received, then the API marks the request **مدفوع**, calculates paid/remaining %, posts the Odoo invoice, registers the payment, and sends the invoice PDF. Opening a request that is already paid locally also posts/pays a leftover Odoo draft. Gemini classifies title/description after that (failure does not keep the request on بانتظار الدفع); English briefs are translated to Arabic with Google Translate before ClickUp.
