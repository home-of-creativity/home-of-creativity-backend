...
| `/social` | Plann-style studio home: connected platforms from `catalog.ts`, three start cards, platform picker, latest posts |
| `/social/compose` `/social/compose/:id` | Plann create: media well, caption, schedule (draft / now / custom), live phone preview. New compose can take `?platforms=facebook,instagram`. Posts (feed, reel, story, album). Facebook + Instagram + Threads + LinkedIn organization feed. New Pages Experience and unsupported Graph story/reel posts map to Arabic/English copy; Instagram uses `/media`; Threads uses `graph.threads.net`; LinkedIn uses the versioned REST `/rest/posts`. |
...
Social nav gated by `social_abilities` (`accounts`, `create`, `approve`, `engage`). Add a network by flipping `live` in `dashboard/src/pages/social/catalog.ts` and wiring the Graph/REST adapter — Home, picker, and compose all read that catalog.
