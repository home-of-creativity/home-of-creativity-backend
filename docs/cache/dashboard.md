...
| Path | Page |
| --- | --- |
| `/` | Overview — one `GET /admin/overview` (counts, `pending_employees`, recent 6). Stats + status donut sit beside the recent table (not stacked full-width). Nav badges use the same endpoint every 30s, not employees/requests. |
