...
طلباتي sends one card per request with Arabic `status_label`, package, paid/remaining, and buttons (10 at a time, then **عرض الأقدم**). Drive files are the client delivery: each file has **تعديل هذه الصورة** and **تعديل الطلب بالكامل**; **اعتماد التسليم** appears after the folder is idle ~2 minutes (or staff deliver). Poll lists the request folder recursively, resolves Google shortcuts (Photos “add”), and also picks loose files sitting in the company folder (not the Hoc Client root). Replacing the same Drive file (new `modifiedTime`/hash) is resent. Revision taps (`revision:` / `revfile:`) are handled first: Telegram alert + ForceReply prompt sent to the user id (works on Laravel-sent Drive photos). Intent is kept even if the client is mid “طلب جديد”; the reason is not stolen as a new title, and API errors (e.g. no files yet) are shown in Arabic. Completed/cancelled folders are not polled. Support notifies sales. Photos become receipts only after **رفع وصل الدفع** or quotation approve, and the hint clears on nav. Catalog period taps are debounced (same package/period within 3 minutes returns the existing request). `ops:poll-drive` every minute over live folders (limit 200), or `ops:poll-drive --file={id}` / `--request={id}`. n8n webhook `POST /webhook/hoc-drive-file` and Google `changes.watch` → `POST /api/integrations/drive/changed` send the uploaded file immediately — dashboard **تحديث الإرسال** is only a fallback. Default API `:8000`. The staff request page shows whether each Drive file reached the client bot. The **client bot must be running** for revision buttons; Drive send itself is Laravel → Telegram.  
